...
static const double COOR_DIFF = 0.00010;
// ~10 meters
...
// Current Location Coordinate as range
...
- (NSString *) getPath:(double)latitude withLongitude:(double)longitude{
    // HTTPS
    return [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/geocode/xml?latlng=%.20f,%.20f&sensor=true", latitude, longitude];
}
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
    if(self.foundRoadName && self.counter <= 2){
        ...
        [Utility debugLog:[NSString stringWithFormat:@"RoadName:%@", string] withBelong:@"GoogleGeo"];
